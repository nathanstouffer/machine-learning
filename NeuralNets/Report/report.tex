...
A MLP is trained using Stochastic Gradient Descent.

To use a trained MLP, begin by using a query example as the input layer where the value of each feature corresponds to a node. Note that categorical data is split into $k$ nodes where $k$ is the number of levels of the attribute with a binary indicator if the example has the level. 
The nodes are then fed forward to the first hidden layer, where a linear sum and then the chosen activation function is applied. Feeding forward continues until the output layer is reached. Here, the appropriate value is selected and returned as the prediction.
